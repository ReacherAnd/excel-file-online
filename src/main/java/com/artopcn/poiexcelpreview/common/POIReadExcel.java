...
public class POIReadExcel {
    ...
    private static String[] table = new String[5];//多个sheet 默认五个sheet.用来将每个sheet的table字符串
//    private static List<Map<String,String>> list = new ArrayList<>();
     ...
     */
    public static Map<String, String> excelWriteToHtml(String sourcePath) {
        File sourceFile = new File(sourcePath);
        try {
            InputStream fis = new FileInputStream(sourceFile);
            // String excelHtml = POIReadExcel.readExcelToHtml(fis, true);
            tableMap = POIReadExcel.readExcelToHtml(fis, true);
            return tableMap;
     ...
     * @return <table>...</table> 字符串
     */
    public static Map<String, String> readExcelToHtml(String filePath, boolean isWithStyle) {
        InputStream is = null;
        try {
            File sourcefile = new File(filePath);
            is = new FileInputStream(sourcefile);
            Workbook wb = WorkbookFactory.create(is);
            tableMap = readWorkbook(wb, isWithStyle);
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            try {
    ...
    /**
     * 程序入口方法（将输入的excel文件流读取成字符串）
     *
     * @param is          excel转换成的输入流
     * @param isWithStyle 是否需要表格样式 包含 字体 颜色 边框 对齐方式
     * @return <table>...</table> 字符串
     */
    public static Map<String, String> readExcelToHtml(InputStream is, boolean isWithStyle) {
        try {
            Workbook wb = WorkbookFactory.create(is);
            tableMap = readWorkbook(wb, isWithStyle);
        } catch (Exception e) {
     ...
     * @return
     */
    private static Map<String, String> readWorkbook(Workbook wb, boolean isWithStyle) {
        if (wb instanceof XSSFWorkbook) {
            XSSFWorkbook xWb = (XSSFWorkbook) wb;
            tableMap = getExcelInfo(xWb, isWithStyle);
        } else if (wb instanceof HSSFWorkbook) {
            HSSFWorkbook hWb = (HSSFWorkbook) wb;
            tableMap = getExcelInfo(hWb, isWithStyle);
        }
        return tableMap;
    }
     ...
     * @return
     */
    public static Map<String, String> getExcelInfo(Workbook wb, boolean isWithStyle) {


        for (int sheetNum = 0; sheetNum < wb.getNumberOfSheets(); sheetNum++) {
            Sheet sheet = wb.getSheetAt(sheetNum);//依次获取Sheet的内容
            ...
            for (int rowNum = sheet.getFirstRowNum(); rowNum <= lastRowNum; rowNum++) {
                ...
                if (null == row) {
                    sb.append("<tr><td Style ='border:solid #000000 1px'>  </td></tr>");
                    continue;
                ...
                for (int colNum = 0; colNum < lastColNum; colNum++) {
                    ...
                    if ((sheetIndexPicMap != null && !sheetIndexPicMap.containsKey(imageRowNum)
                            || sheetIndexPicMap == null) && cell == null) {
                        //特殊情况 空白的单元格会返回null+//判断该单元格是否包含图片，为空时也可能包含图片
                        sb.append("<td Style ='border:solid #000000 1px'>  </td>");
                        continue;
                    }
                    if (sheetIndexPicMap != null && sheetIndexPicMap.containsKey(imageRowNum)) {
                        //待修改路径
            ...
            excelMap.put(sheetName,tableHtml);
            list.add(excelMap);*/

            tableMap.put(sheetName,tableHtml);
        }
        return tableMap;
    }
    ...
    private static void dealExcelStyle(Workbook wb, Sheet sheet, Cell cell, StringBuffer sb) {
        ...
        if (cellStyle != null) {
            ...
            if (wb instanceof XSSFWorkbook) {
                ...
                mergeCell(sheet, cell, sb, topRow, topColumn, xf.getFontHeight(), xf);
                ...
                int topRow = cell.getRowIndex(), topColumn = cell.getColumnIndex();
                mergeCell(sheet, cell, sb, topRow, topColumn, hf.getFontHeight(), hf);
     ...
     * @param hf
     */
    public static void mergeCell(Sheet sheet, Cell cell, StringBuffer sb, int topRow, int topColumn, short fontHeight, Font hf) {
        if (map[0].containsKey(topRow + "," + topColumn)) {//该单元格为合并单元格，宽度需要获取所有单元格宽度后合并
            ...
            if (topColumn != bottomColumn) {//合并列，需要计算相应宽度
                ...
                sb.append("width:" + columnWidth / 256 * fontHeight / 20 + "pt;");
            } else {
    ...
    static String[] borderStyles = {"solid ", "solid ", "solid ", "solid ", "solid ", "solid ", "solid ",
